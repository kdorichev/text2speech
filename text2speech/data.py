...
__all__ = ['get_files', 'get_audio_files', 'get_txt_files', 'words_count', 'label_func', 'ReadTxt']
...
def words_count(files, sort=True, drop_punkt=True):
    ...
    for file in files:
        ...
        for w in wt.tokenize((russian_cleaner(text))):
            if drop_punkt and w in _punct + "». », !», ?» »:": continue
            if w not in wc:
                wc[w] = 1
            else:
                wc[w] += 1
